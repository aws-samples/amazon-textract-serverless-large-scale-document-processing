...
def startJob(
    bucketName, objectName, documentId, snsTopic, snsRole, detectForms, detectTables
):

    print(
        "Starting job with documentId: {}, bucketName: {}, objectName: {}".format(
    ...
    if not detectForms and not detectTables:
        response = client.start_document_text_detection(
            ...
            DocumentLocation={"S3Object": {"Bucket": bucketName, "Name": objectName}},
            NotificationChannel={"RoleArn": snsRole, "SNSTopicArn": snsTopic},
            JobTag=documentId,
        )
    else:
        features = []
...
def changeVisibility(sqs, qUrl, receipt_handle):
    try:
        sqs.change_message_visibility(
            QueueUrl=qUrl, ReceiptHandle=receipt_handle, VisibilityTimeout=0
        )
    except Exception as e:
        print(
            "Failed to change visibility for {} with error: {}".format(
                receipt_handle, e
            )
        )
...
def getMessagesFromQueue(
    ...
    response = sqs.receive_message(
        QueueUrl=qUrl, MaxNumberOfMessages=1, VisibilityTimeout=60  # 14400
    )
    ...
    if messages:
        ...
        for message in messages:
            ...
            try:
                ...
                changeVisibility(sqs, qUrl, receipt_handle)
                if (
                    e.__class__.__name__ == "LimitExceededException"
