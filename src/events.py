...
def S3BatchOperationsEvent(bucketArn, objectName):
    return {
        ...
        "tasks": [{"taskId": "1", "s3Key": objectName, "s3BucketArn": bucketArn}],
    }
...
def s3Event(bucketName, objectName):
    return {
        "Records": [
            {"s3": {"bucket": {"name": bucketName}, "object": {"key": objectName}}}
        ]
    }
...
def jobResultsEvent(jobId, jobTag, jobStatus, jobAPI, bucketName, objectName):
    ...
    message = {
        ...
        "Status": jobStatus,
        "API": jobAPI,
        "DocumentLocation": {"S3Bucket": bucketName, "S3ObjectName": objectName},
    }
...
def searchEvent(keyword):
    ...
    return {"resource": "/search", "queryStringParameters": {"k": keyword}}
...
def createDocumentEvent(bucketName, objectName):
    ...
    return {
        "resource": "/document",
        "queryStringParameters": {"bucketname": bucketName, "objectName": objectName},
    }
...
def getDocumentEvent(documentId):
    ...
    return {
        "resource": "/document",
        "queryStringParameters": {"documentid": documentId},
    }
